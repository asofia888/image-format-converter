...
// FIX: Import ResizeConfig from '../types' instead of '../hooks/useImageConverter'.
...
const ConversionControls: React.FC<ConversionControlsProps> = ({
  ...
  }, [onSavePreset, newPresetName, setNewPresetName, setIsSavingPreset]);
        ...
        {!isSavingPreset ? (
            <div className="flex gap-2">
             ...
             <div className="flex gap-2 items-center bg-slate-100 dark:bg-slate-900/50 p-2 rounded-md">
                ...
                />
                <button onClick={handleConfirmSavePreset} className="px-4 py-2 text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700 rounded-md transition-colors">{t('saveButton')}</button>
                <button onClick={handleCancelSavePreset} className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-md transition-colors">{t('cancelButton')}</button>
            </div>
        )}
      </div>
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto] gap-4 items-end transition-opacity ${resizeConfig.enabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  disabled={!resizeConfig.enabled}
                  className="p-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                >
                  <Icon name={resizeConfig.maintainAspectRatio ? 'lockClosed' : 'lockOpen'} className="w-5 h-5 text-slate-700 dark:text-slate-300" />
                </button>
              </div>
              {originalDimensions && (
